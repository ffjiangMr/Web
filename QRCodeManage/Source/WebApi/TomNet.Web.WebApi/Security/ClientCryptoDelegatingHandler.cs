...
using TomNet.Utility.Exceptions;
using TomNet.Utility.Logging;
using TomNet.Utility.Secutiry;
using TomNet.Web.WebApi.Internal;
using TomNet.Web.WebApi.Properties;

namespace TomNet.Web.WebApi.Security
{
    /// <summary>
    /// 客户端通信加密解密消息处理器
    /// </summary>
    public class ClientCryptoDelegatingHandler : DelegatingHandler
    {
        ...
        {
            request.Headers.Add(HttpHeaderNames.TomNetClientPublicKey, _clientPublicKey);
        ...
        private HttpResponseMessage DecryptResponse(HttpResponseMessage response)
        {
            ...
            try
            {
                ...
                if (data == null)
                {
                    throw new TomNetException(Resources.Http_Security_Client_VerifyResponse_Failt);
                }
                HttpContent content = new StringContent(data);
